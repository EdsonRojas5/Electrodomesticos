...
@Stateless
@Dependent
public class DaoElectrodomestico {
  ...
  public List<Electrodomestico> consulta() {
    /* Con el resultado de la consulta se llena una lista con objetos de la
     * clase "Pasatiempo". */
    return em.createQuery("SELECT c FROM Electrodomestico c ORDER BY c.nombre",
        Electrodomestico.class).getResultList();
  }
  public Electrodomestico busca(Integer id) {
    return em.find(Electrodomestico.class, id);
  }
  public void agrega(Electrodomestico modelo) {
    em.persist(modelo); // Agrega el modelo a la base de datos.
  }
  public void modifica(Electrodomestico modelo) {
    em.merge(modelo);// Guarda los cambios al modelo.
  }
  public void elimina(Electrodomestico modelo) {
    // Busca el modelo en base a su id.
    final Electrodomestico anterior = em.find(Electrodomestico.class, modelo.getId());
